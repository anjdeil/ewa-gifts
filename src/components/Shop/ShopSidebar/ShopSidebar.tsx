...
const ShopSidebar: FC<ShopSidebarPropsType> = ({
  priceRange,
  availableAttributes,
}) => {
  ...
  const categories =
    categoriesData?.data && (categoriesData.data.items as CategoryType[]);
  const currentCategory = categories?.find(
    (category: CategoryType) => category.slug === categorySlug
  );
  ...
  const colorAttribute = availableAttributes.find(
    ({ slug }) => slug === "base_color"
  );
  const colors = colorAttribute?.options;
  const currentColor =
    searchParams.get("attribute") === "pa_base_color"
      ? searchParams.get("attribute_term")
      : null;
  ...
  const [minPrice, setMinPrice] = useState(
    searchParams.get("min_price") || String(priceRange.min)
  );
  const [maxPrice, setMaxPrice] = useState(
    searchParams.get("max_price") || String(priceRange.max)
  );
  ...
  useEffect(() => {
    ...
    if (
      newPriceParams?.min_price !== min_price ||
